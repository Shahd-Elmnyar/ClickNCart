                        ...
                        <div class="block-4 text-center border">
                            <figure class="block-4-image">
                                <a href="{{ route('products.show', $item->id) }}">
                                    <img src="{{ asset('storage/' . $item->img) }}" alt="{{ $item->name[auth()->check() ? auth()->user()->locale : 'en'] ?? $item->name['en'] }}" class="img-fluid">
                                </a>
                            </figure>
                            <div class="block-4-text p-4">
                                <h3><a href="{{ route('products.show', $item->id) }}">{{ $item->name[auth()->check() ? auth()->user()->locale : 'en'] ?? $item->name['en'] }}</a></h3>
                                <p class="mb-0">{{ Str::limit($item->content[auth()->check() ? auth()->user()->locale : 'en']??$item->content['en'], 50) }}</p>
